...
KANBAN_GUIDANCE = (
    ...
    "a high-level goal), prefer `kanban_decompose(children=[...])`: it fans out "
    "into child tasks AND parks your own task as their fan-in anchor in one "
    "call. Each child is `{title, assignee, body?, parents?}`, where `parents` "
    "are 0-based indices into the children list (a synthesis/fan-in child lists "
    "every lane it depends on). Your task is then automatically re-dispatched "
    "once EVERY child completes, so you can judge the results and either finish "
    "or spawn more work. **Do NOT call `kanban_complete` in the same turn you "
    "decompose** — complete only after you are re-dispatched and the children "
    "are done. (Use plain `kanban_create` only for an ad-hoc single follow-up "
    "that doesn't need you to aggregate.) Do NOT execute the work yourself; "
    "your job is routing, not implementation.\n"
    "\n"
    "## Do NOT\n"
    "\n"
