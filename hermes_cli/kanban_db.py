...
def reap_worker_zombies() -> "list[int]":
    ...
    """
    # Gate the POSIX reap loop behind ``os.name != "nt"``: Windows has no
    # ``os.waitpid(-1, ...)`` group-wait, and the Windows-native-support
    # guard test asserts this exact gate sits in front of the call.
    reaped: "list[int]" = []
    if os.name != "nt":
        try:
            while True:
...
def detect_crashed_workers(conn: sqlite3.Connection) -> list[str]:
    ...
    # Fingerprint errors across this batch so a systemic failure (the same
    # error recurring on >=3 tasks) trips the breaker immediately instead of
    # burning the full per-task retry budget. ``protocol_violation`` and
    # provider-failure crashes already carry ``failure_limit=1`` from the
    # producer; systemic detection only escalates the otherwise-retryable
    # ``failure_limit is None`` crashes.
    _fp_counts: dict[str, int] = {}
    for _d in crash_details:
        _fp = _error_fingerprint(str(_d.get("error_text") or ""))
        _fp_counts[_fp] = _fp_counts.get(_fp, 0) + 1
    for detail in crash_details:
        ...
        if failure_limit is None and not detail.get("protocol_violation"):
            # Systemic: same fingerprint seen on >=3 crashed tasks this pass.
            if _fp_counts.get(_error_fingerprint(error_text), 0) >= 3:
                failure_limit = 1
    ...
    with write_txn(conn):
        conn.execute(
            ...
            (
                ...
                mode, origin_session_id, origin_profile, origin_context,
                request_id, notifier_profile, now,
            ),
        )
        if notifier_profile:
