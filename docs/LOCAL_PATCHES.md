...
- **Configurable per-outcome reactions** — Telegram lifecycle reactions resolve through a slot map (`progress`/`success`/`failure`); defaults unchanged from upstream (👀→👍/👎), each overridable via `telegram.reaction_{progress,success,failure}` / env `TELEGRAM_REACTION_{PROGRESS,SUCCESS,FAILURE}`. An empty value clears instead of sets — deploy uses `reaction_success: ""` so a completed turn clears the 👀 rather than stamp a repetitive 👍. Additive (upstream reaction tests unchanged); local tests in `tests/local/gateway/test_telegram_reactions_config.py`.
