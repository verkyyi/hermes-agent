...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if function_name == "todo":
        ...
        # Pass the full surface context (origin-return, local patch #1) so
        # async Kanban/telemetry events can route a completion back to the
        # requesting surface and correlate with the foreground turn.
        return _ra().handle_function_call(
            ...
            user_id=agent._user_id or "",
            request_id=(getattr(_ra().current_turn(), "id", "") if _ra().current_turn() is not None else ""),
            user_task=getattr(agent, "_current_user_message_for_tools", ""),
