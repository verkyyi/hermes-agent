...
- **HermesBench consolidated benchmark** — one runner (`evals/hermesbench/`) wrapping the fork's four eval harnesses (responsiveness, kanban-scale, orchestrator-routing, origin-return) behind a registry + SQLite trend store (`$HERMES_HOME/hermesbench.db`, rollback-journal not WAL) + a daily summary with deltas vs the prior run. no tier concept — every suite runs each time; the model-backed suites (orchestrator, origin_return) self-skip without `HERMES_RUN_LLM_EVALS`. Records git/model/profile fingerprint per run (harness pinning). Category + 3-grading-mode design inspired by ClawBench Core v1. Trend view is a bundled dashboard plugin (`plugins/hermesbench/dashboard/`, tab `/hermesbench` + `GET /api/plugins/hermesbench/trend`): single overall-score line + per-category trend charts. Daily launchd job `ai.hermes.hermesbench` (host artifact, @ 04:00 with `HERMES_RUN_LLM_EVALS=1`). Run: `python -m evals.hermesbench.run`. Tests: `tests/hermesbench/`. Doc: `evals/hermesbench/README.md`. Backward-compatible one-line tweak to `tests/stress/test_benchmarks.py` (honor `KANBAN_BENCH_OUT`, return results).
