...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Stable AgentFeeds inventory (local patch #9). Lives in the shared
    # system-prompt assembly so CLI, gateway, cron, and worker sessions all
    # get the same cached-prefix stream manifest when enabled. It deliberately
    # excludes volatile freshness/health/content; plugins may inject those per
    # turn. Imported lazily off ``run_agent`` to avoid a circular import and so
    # tests that monkeypatch ``run_agent._build_agentfeeds_system_manifest``
    # still take effect.
    try:
        import run_agent as _run_agent
        _agentfeeds_manifest = _run_agent._build_agentfeeds_system_manifest(
            {"agentfeeds": {"system_prompt": getattr(agent, "_agentfeeds_system_prompt_config", {})}}
        )
    except Exception:
        _agentfeeds_manifest = None
