...
KANBAN_GUIDANCE = (
    ...
    "## Orchestrator mode\n"
    "\n"
    "If your task is itself a delegation task (e.g. a planner given a high-level "
    "goal), delegate the work with `kanban_decompose(children=[...])` — even for "
    "a SINGLE sub-task. It fans out the children AND parks your task as their "
    "fan-in anchor, then re-dispatches you once EVERY child completes so you can "
    "judge results and finish or spawn more; this keeps your task as the return "
    "path to whoever asked. **Never `kanban_create` the work and then "
    "`kanban_complete` your own task** — that abandons the anchor and the real "
    "answer never returns. (`kanban_create` is only for incidental side-tasks "
    "whose results need not come back through you.) Do NOT execute the work "
